...
if [[ -d /volume1/docker/autospy  ]]; then
	docker stop auto_spy
	docker rm auto_spy
	echo "autospy配置文件已存在"
	docker run -dit --restart=always --name=auto_spy --log-opt max-size=100m --log-opt max-file=3 -v /volume1/docker/autospy:/autospy --hostname=auto_spy xieshang1111/auto_spy:$arch
else
	mkdir auto_spy_data
    docker run -dit --restart=always --name=auto_spy --hostname=auto_spy xieshang1111/auto_spy:$arch
	docker cp auto_spy:/autospy/ /volume1/docker/autospy
	docker stop auto_spy
	docker rm auto_spy	
	...
	if [[ $dir != "" ]];then
	docker run -dit --restart=always --name=auto_spy --log-opt max-size=100m --log-opt max-file=3 -v /volume1/docker/autospy:/autospy --hostname=auto_spy xieshang1111/auto_spy:$arch
	cp $dir/auto_spy.yaml /volume1/docker/autospy
	docker exec -it auto_spy bash /autospy/start.sh
	exit 0
else
	mkdir auto_spy_data
	...
	docker rm auto_spy
	docker run -dit --restart=always --name=auto_spy --log-opt max-size=100m --log-opt max-file=3 -v /volume1/docker/autospy:/autospy --hostname=auto_spy xieshang1111/auto_spy:$arch
	cp /volume1/docker/autospy/auto_spy_simple.yaml /volume1/docker/autospy/auto_spy.yaml
